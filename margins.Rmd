...
Below is a list of each hospital's 2017 total revenue, net income/loss, and their margin. 

<br>

```{r prep 2017 margin table for each hospital, include = FALSE}
har.margin.2017.table <- master.har %>%
  ...
  select(3,4,5,8,10,11,13,12) %>%
  mutate(Margins = net.income/total.revenue)
```

```{r table of 2017 margins for each hospital}
datatable(har.margin.2017.table, class = "cell-border stripe", filter = "top", rownames = FALSE,
...
Not suprisingly, given the number of hospitals that recorded a net loss in Southwest Minnesota for 2017, that region also has the highest number and percentage of hospitals that fall below the 4% margin threshold. Northeast also has a significanlty high number of hospitals below the threshold.
...
har.margins.pr.plot <- ggplot(filter(har.margins, year == 2017), aes(planning.region, margins, color = planning.region)) +
        geom_point_interactive(size = 3, aes(data_id = margins, tooltip = paste(planning.region, "\nHospital Name: ", `Hospital Name`, "\nType of hospital: ", CAH, "\nTotal revenue: ", dollar(total.revenue), "\nNet income: ", dollar(income.loss), "\nMargin: ", percent(margins), sep = ""))) +
          geom_label(data = filter(har.margins.threshold.pr, year == 2017), size = 3, show.legend = FALSE, aes(x = planning.region, y = -.3, label = paste(below.threshold, " hospitals\nunhealthy margins\n", percent(pct.below.threshold), " of hospitals", sep = ""))) +
          geom_label(data = filter(har.margins.avg.pr, year == 2017), aes(x = planning.region, y = .4, label = paste("Avg margins\n", percent(avg.margins), sep = "")), size = 3, show.legend = FALSE) +
          geom_hline(yintercept = 0.04, color = "black") +
        labs(x="", y = "", color="", title = "Margins for each hospital in 2017")+
...
har.margins.ruca.plot <- ggplot(filter(har.margins, year == 2017), aes(Dem_Desc, margins, color = Dem_Desc)) +
        geom_point_interactive(size = 3, aes(data_id = margins, tooltip = paste(Dem_Desc, "\nHospital Name: ", `Hospital Name`, "\nType of hospital: ", CAH, "\nTotal revenue: ", dollar(total.revenue), "\nNet income: ", dollar(income.loss), "\nMargin: ", percent(margins), sep = ""))) +
          geom_label(data = filter(har.margins.threshold.ruca, year == 2017), size = 3, show.legend = FALSE, aes(x = Dem_Desc, y = -.3, label = paste(below.threshold, " hospitals\nunhealthy margins\n", percent(pct.below.threshold), " of hospitals", sep = ""))) +
          geom_label(data = filter(har.margins.avg.ruca, year == 2017), aes(x = Dem_Desc, y = .4, label = paste("Avg margins\n", percent(avg.margins), sep = "")), size = 3, show.legend = FALSE) +
          geom_hline(yintercept = 0.04, color = "black") +
        labs(x="", y = "", color="", title = "Margins for each hospital in 2017")+
        scale_y_continuous(labels=scales::percent,
                           limits = c(-.4, .3))+
...
## 2017 individual hospitals {.tabset}
...
har.margins.cah.pr.plot <- ggplot(filter(har.margins, year == 2017), aes(planning.region, margins, color = CAH)) +
    geom_point_interactive(size = 3, aes(data_id = margins, tooltip = paste(planning.region, "\nHospital Name: ", `Hospital Name`, "\nType of hospital: ", CAH, "\nTotal revenue: ", dollar(total.revenue), "\nNet income/loss: ", dollar(income.loss), "\nMargins: ", percent(margins), sep = ""))) +
    geom_label(data = filter(har.cah.margins.threshold.pr, year == 2017, CAH == "CAH"), size = 2, show.legend = FALSE, aes(x = planning.region, y = -.3, label = paste("CAH below 4%\n", below.threshold, " (", percent(pct.below.threshold), ")", sep = ""))) +
    geom_hline(yintercept = 0.04, color = "black") +
    geom_label(data = filter(har.cah.margins.threshold.pr, year == 2017, CAH == "Non-CAH"), size = 2, show.legend = FALSE, aes(x = planning.region, y = .3, label = paste("Non-CAH below 4%\n", below.threshold, " (", percent(pct.below.threshold), ")", sep = ""))) +
